...
class LoginController extends Controller
{
    ...
    */
    ...
    public function attemptLogin(Request $request) {
            // function called to see whether the user is allowed based on the dataset of allowed users
            $userAllowed = $this->userExists($request);
    ...
    public function loggin(Request $request){
                $ldapUser = Adldap::search()->where('sAMAccountName', $request->cpf)->firstOrFail();
                    ...
                    if($ldapUser){
                    //     // if the corporate user exists and is also allowed in the list of users, save the data in the table else not
                        if ($this->userExists($request) == "Yes"){
    ...
    public function userExists(Request $request){
        ...
        foreach (\App\AllowedUsers::all() as $user) {
